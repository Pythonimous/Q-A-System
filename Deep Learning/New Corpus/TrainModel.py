...
with open('TrainingFeatures.pkl', 'rb') as trainfile:
...
# 2. Feature scaling
sc = StandardScaler()
X_train = [sc.fit_transform(i) for i in features] #scaling matrices
X_test = [sc.fit_transform(i) for i in testfeatures] #scaling matrices

X_train = np.array([i.reshape((1,)+i.shape) for i in X_train]) #adding "step" dimensions
X_test = np.array([i.reshape((1,)+i.shape) for i in X_test])

X_train = X_train.reshape(2086,340,8,1) #reordering correctly
